...
internal sealed class CRuntimeGenerator
{
        ...
        Func<PrefabSymbol, bool>? includePrefab = null)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("typedef struct flx_file_task_state flx_file_task_state;");
        builder.AppendLine("typedef struct flx_file_task {");
        builder.AppendLine("    flx_file_task_state *state;");
        builder.AppendLine("} flx_file_task;");
        builder.AppendLine();
        builder.AppendLine("flx_file_task flx_task_read_file_start(const char *path);");
        builder.AppendLine("int flx_task_poll_file(flx_file_task *task);");
        builder.AppendLine("int flx_task_file_status(const flx_file_task *task);");
        builder.AppendLine("usize flx_task_file_length(const flx_file_task *task);");
        builder.AppendLine("const char *flx_task_file_data(const flx_file_task *task);");
        builder.AppendLine("void flx_task_destroy_file(flx_file_task *task);");
        builder.AppendLine();
    ...
    public string GenerateSource(CompilationModel model)
    {
        ...
        builder.AppendLine("    const char *configured = getenv(\"FLX_WORKERS\");");
        builder.AppendLine("    if (configured != NULL) {");
        builder.AppendLine("        while (*configured == ' ' || *configured == '\\t') configured++;");
        builder.AppendLine("        if (*configured >= '1' && *configured <= '9') {");
        builder.AppendLine("            char *end = NULL;");
        builder.AppendLine("            unsigned long requested = strtoul(configured, &end, 10);");
        builder.AppendLine("            while (*end == ' ' || *end == '\\t') end++;");
        builder.AppendLine("            if (end != configured && *end == '\\0' && requested > 0) return (usize)requested;");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine("#if defined(_WIN32)");
        ...
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("struct flx_file_task_state {");
        builder.AppendLine("    void *handle;");
        builder.AppendLine("    volatile int done;");
        builder.AppendLine("    int status;");
        builder.AppendLine("    char *path;");
        builder.AppendLine("    char *data;");
        builder.AppendLine("    usize length;");
        builder.AppendLine("};");
        builder.AppendLine();
        builder.AppendLine("static char *flx_strdup_cstr(const char *value) {");
        builder.AppendLine("    if (value == NULL) value = \"\";");
        builder.AppendLine("    usize len = (usize)strlen(value);");
        builder.AppendLine("    char *copy = (char *)malloc(len + 1);");
        builder.AppendLine("    if (copy == NULL) abort();");
        builder.AppendLine("    memcpy(copy, value, len + 1);");
        builder.AppendLine("    return copy;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("static int flx_read_entire_file(const char *path, char **out_data, usize *out_length) {");
        builder.AppendLine("    *out_data = NULL;");
        builder.AppendLine("    *out_length = 0;");
        builder.AppendLine("    FILE *file = fopen(path, \"rb\");");
        builder.AppendLine("    if (file == NULL) return -1;");
        builder.AppendLine("    if (fseek(file, 0, SEEK_END) != 0) { fclose(file); return -2; }");
        builder.AppendLine("    long size = ftell(file);");
        builder.AppendLine("    if (size < 0) { fclose(file); return -3; }");
        builder.AppendLine("    if (fseek(file, 0, SEEK_SET) != 0) { fclose(file); return -4; }");
        builder.AppendLine("    char *data = (char *)malloc((usize)size + 1);");
        builder.AppendLine("    if (data == NULL) abort();");
        builder.AppendLine("    usize read = size == 0 ? 0 : fread(data, 1, (usize)size, file);");
        builder.AppendLine("    fclose(file);");
        builder.AppendLine("    if (read != (usize)size) { free(data); return -5; }");
        builder.AppendLine("    data[read] = '\\0';");
        builder.AppendLine("    *out_data = data;");
        builder.AppendLine("    *out_length = read;");
        builder.AppendLine("    return 0;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("static void flx_task_read_file_sync(flx_file_task_state *state) {");
        builder.AppendLine("    state->status = flx_read_entire_file(state->path, &state->data, &state->length);");
        builder.AppendLine("    state->done = 1;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("#if defined(_WIN32)");
        builder.AppendLine("static unsigned __stdcall flx_task_file_worker(void *arg) {");
        builder.AppendLine("    flx_task_read_file_sync((flx_file_task_state *)arg);");
        builder.AppendLine("    return 0;");
        builder.AppendLine("}");
        builder.AppendLine("#endif");
        builder.AppendLine();
        builder.AppendLine("flx_file_task flx_task_read_file_start(const char *path) {");
        builder.AppendLine("    flx_file_task task = { 0 };");
        builder.AppendLine("    flx_file_task_state *state = (flx_file_task_state *)calloc(1, sizeof(flx_file_task_state));");
        builder.AppendLine("    if (state == NULL) abort();");
        builder.AppendLine("    state->path = flx_strdup_cstr(path);");
        builder.AppendLine("    task.state = state;");
        builder.AppendLine("#if defined(_WIN32)");
        builder.AppendLine("    uintptr_t thread = _beginthreadex(NULL, 0, flx_task_file_worker, state, 0, NULL);");
        builder.AppendLine("    if (thread == 0) {");
        builder.AppendLine("        flx_task_read_file_sync(state);");
        builder.AppendLine("    } else {");
        builder.AppendLine("        state->handle = (void *)thread;");
        builder.AppendLine("    }");
        builder.AppendLine("#else");
        builder.AppendLine("    flx_task_read_file_sync(state);");
        builder.AppendLine("#endif");
        builder.AppendLine("    return task;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("int flx_task_poll_file(flx_file_task *task) {");
        builder.AppendLine("    if (task == NULL || task->state == NULL) return 1;");
        builder.AppendLine("#if defined(_WIN32)");
        builder.AppendLine("    if (!task->state->done && task->state->handle != NULL) {");
        builder.AppendLine("        DWORD result = WaitForSingleObject((HANDLE)task->state->handle, 0);");
        builder.AppendLine("        if (result == WAIT_OBJECT_0) task->state->done = 1;");
        builder.AppendLine("    }");
        builder.AppendLine("#endif");
        builder.AppendLine("    return task->state->done;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("int flx_task_file_status(const flx_file_task *task) {");
        builder.AppendLine("    return task == NULL || task->state == NULL ? -1000 : task->state->status;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("usize flx_task_file_length(const flx_file_task *task) {");
        builder.AppendLine("    return task == NULL || task->state == NULL ? 0 : task->state->length;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("const char *flx_task_file_data(const flx_file_task *task) {");
        builder.AppendLine("    return task == NULL || task->state == NULL || task->state->data == NULL ? \"\" : task->state->data;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("void flx_task_destroy_file(flx_file_task *task) {");
        builder.AppendLine("    if (task == NULL || task->state == NULL) return;");
        builder.AppendLine("#if defined(_WIN32)");
        builder.AppendLine("    if (task->state->handle != NULL) {");
        builder.AppendLine("        WaitForSingleObject((HANDLE)task->state->handle, INFINITE);");
        builder.AppendLine("        CloseHandle((HANDLE)task->state->handle);");
        builder.AppendLine("    }");
        builder.AppendLine("#endif");
        builder.AppendLine("    free(task->state->path);");
        builder.AppendLine("    free(task->state->data);");
        builder.AppendLine("    free(task->state);");
        builder.AppendLine("    task->state = NULL;");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("void flx_array_string_init(flx_array_string *a) {");
